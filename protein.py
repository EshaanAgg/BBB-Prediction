...
df_stitch = pd.read_csv("9606.protein_chemical.links.detailed.v5.0.tsv", sep="\t")
...
df_drugpro = df_drugpro[df_drugpro["combined_score"] >= 500].reset_index(drop=True)
df_drugpro = df_drugpro.join(df_protein.set_index("protein"), on="protein")
df_drugpro = df_drugpro.dropna().reset_index(drop=True)
df_drugpro.rename(
    columns={"chemical": "Drug_ID", "preferred_name": "Gene_ID"}, inplace=True
)
...
carrier_gene = df_hugo[df_hugo["NCBI Gene ID"].astype(str).isin(carrier_gene_id)][
    "Approved symbol"
]
carrier_gene = carrier_gene[
    ~(carrier_gene.str.startswith("SLC") | carrier_gene.str.startswith("ABC"))
].values

df_drugpro["Relation"][df_drugpro["Gene_ID"].isin(carrier_gene)] = 3

df_drugpro = df_drugpro[
    ~df_drugpro.duplicated(subset=["Drug_ID", "Gene_ID"])
].reset_index(drop=True)

gene_node = df_drugpro["Gene_ID"].unique()
df_gene_node = pd.DataFrame(
    {
        "Gene_ID": gene_node,
        "Node_Idx": range(df_drug.shape[0], df_drug.shape[0] + len(gene_node)),
    }
)
...
df_drugpro.to_csv("protein.csv", index=False)

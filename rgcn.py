...
def model_val(val_idx, plus_test=False):
    ...
    if plus_test == False:
        ...
        spec_val = recall_score(
            data.y[val_idx].cpu(), pred_val.cpu(), pos_label=0)
        ...
        auroc_test = roc_auc_score(
            data.y[test_index].cpu(), out_test[:, 1].cpu())
        ...
        spec_test = recall_score(
            data.y[test_index].cpu(), pred_test.cpu(), pos_label=0)
...
train_slice, val_slice = next(
    sss.split(data.x[train_index].cpu(), data.y[train_index].cpu()))
...
for epoch in range(500):
    ...
    auroc_val, acc, sens, spec, mcc, auroc, auprc = model_val(
        val_index, plus_test=True)
